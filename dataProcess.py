...
def getLociDataset(tsvFile, db_name, default_enc):

    """TP0136 = (161161, 162888)
    TP0548 = (606728, 607933)
    TP0705 = (789021, 790067)
    SrRNA = (233095, 236053)
    man_loci = [TP0136, TP0548, TP0705, SrRNA]
    loci_pos = []
    for locus in man_loci:
        locus_start = locus[0]
        locus_end = locus[1]
        loci_pos.extend(range(locus_start, locus_end))
    loci_pos.sort()"""

    # read MLST data from file
    loci_df = pd.read_csv("MLST.tsv", sep='\t')
    loci_pos = loci_df["location"].head().tolist()
    loci_pos = [ast.literal_eval(x) for x in loci_pos]
    loci_pos = [x for xs in loci_pos for x in xs]
    loci_pos.sort()
    ...
    # find common SNP positions in reference database and query
    query_pos = query.Position.tolist()
    common_pos = list(set(loci_pos).intersection(ref_positions))
    common_pos = list(set(map(str, common_pos)).intersection(query_pos))
    common_pos.sort(key=lambda pos: int(pos))
...
# getLociDataset("variantContentTable.tsv", "snps.db", [0, 0, 0, 0, 1, 0])
