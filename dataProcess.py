...
def newEncQuery(df, default_enc, common_pos, filter):
    if isinstance(default_enc, list):
        symb_count = len(default_enc)
    else:
        symb_count = 1

    sample_names = df["SAMPLES"].apply(lambda x: x.split(",")).tolist()
    ...
    for position in df['Position']:
        ...
        info = df[df['Position'] == position]["INFO"]
        impact = info.values[0].split("IMPACT=")[1].split(",")[0]
        if impact in filter:
            sequences = df[df['Position'] == position]["SAMPLES"].apply(lambda x: x.split(",")).tolist()
            ...
            for sequence in sequences:
                ...
                if '-' not in entry and len(entry) == 1:
                    ...
                    eval_3D[seq][pos] = encodeNuc(entry.lower(), default_enc)
...
""""
old version:
def getLociDataset(tsvFile, db_name, default_enc, loci_list):
    # load loci dataframe
    loci_df = pd.read_csv("loci.tsv", sep='\t', converters={"var sites": ast.literal_eval})
    # create list of MLST loci SNP positions
    MLST_loci = loci_df[loci_df['locus_tag'].isin(loci_list)]
    ...
    all_train_names = SNPdf['Sequence_name'].unique().tolist()
    all_train_names.append("Reference(NC_021490)")
    dataset = {'train': train_loci, 'train_seq_names': train_names, 'all_train_names': all_train_names, 'test': test_loci, 'test_sample_names': sample_names, 'loci': loci_list}
    return dataset"""
